...
export async function registerDevicePushToken() {
  try {
    const isExpoGo = Constants?.appOwnership === 'expo' || Constants?.executionEnvironment === 'storeClient';
    // W Expo Go push notifications nie działają stabilnie / wymagają logowania w Expo.
    // Ale spróbujmy pobrać token jeśli się da.
    ...
    if (token) {
      await api.post('/api/notifications/register-push', { token, platform: Platform.OS }).catch(() => {});
    }
    return token;
  } catch (e) {
...
export async function acknowledgeOverdueItem(type, id, date) {
  try {
    ...
    const key = `overdue:${id}:${dt}`;
    await addAck(key);
    // Also add to expired set just in case
    const baseKey = id; // simplified
    const keyExpired = `expired:${baseKey}:${dt}:${type}`;
    await addAck(keyExpired);
  } catch {}
}
