...
export const handleSupabaseRequest = async (url, method, body, headers, supabase) => {
  ...
  // TOOLS (POST)
  if (route === '/api/tools' && method === 'POST') {
    const { name, sku, quantity, location, category, description, barcode, qr_code, serial_number, serial_unreadable, inventory_number, inspection_date, min_stock, max_stock, is_consumable, manufacturer, model, production_year } = body;
    ...
    if (!serialProvided && !unreadableFlag) {
      throw new Error('Factory serial number is required or mark as unreadable');
    }
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        barcode: barcode || sku,
        qr_code: qr_code || sku,
        serial_number: serialProvided ? serial_number : null,
        serial_unreadable: unreadableFlag ? 1 : 0,
        inventory_number: inventory_number || null,
        inspection_date: inspection_date || null,
        min_stock: min_stock === '' ? null : min_stock,
  ...
  // TOOLS UPDATE (PUT)
  if (route.match(/^\/api\/tools\/\d+$/) && method === 'PUT') {
    const id = route.split('/')[3];
    const { name, sku, quantity, location, category, description, barcode, qr_code, serial_number, serial_unreadable, status, inventory_number, inspection_date, min_stock, max_stock, is_consumable, manufacturer, model, production_year } = body;
    ...
    if (error) throw new Error(error.message);
    return { message: 'Tool updated successfully', item: data };
  }

  // TOOLS DELETE (DELETE)
  if (route.match(/^\/api\/tools\/\d+$/) && method === 'DELETE') {
    const id = route.split('/')[3];
  ...
  // TOOL NOTIFY RETURN (POST)
  if (route.match(/^\/api\/tools\/\d+\/notify-return$/) && method === 'POST') {
    const toolId = route.split('/')[3];
    const { message, target_employee_id } = body;
    ...
    if (target_employee_id) {
        ...
        const { data: targetUser } = await supabase
            ...
            .maybeSingle();
        if (targetUser) recipientUserId = targetUser.id;
    } 
    
    if (!recipientUserId) {
        // Try to find who has the tool issued (active issue)
    ...
    if (!recipientUserId) {
        return { message: 'Nie znaleziono użytkownika przypisanego do pracownika (brak odbiorcy powiadomienia)', success: false };
    }
    ...
    const { error: notifError } = await supabase
        ...
        .insert([{
            ...
            item_type: 'tool',
            item_id: toolId,
            message: message || `Prośba o zwrot narzędzia: ${tool.name}`,
            read: false,
            created_at: new Date().toISOString()
        }]);
    ...
    return { success: true, message: 'Wysłano powiadomienie o zwrot' };
  }
  ...
  // SERVICE HISTORY SUMMARY (GET)
  if (route === '/api/service-history/summary' && method === 'GET') {
    // 1. In Service tools
    const { data: inService, error: inServiceError } = await supabase
        .from('tools')
        .select('id, name, sku, service_quantity, service_order_number, service_sent_at')
        .gt('service_quantity', 0)
        .order('service_sent_at', { ascending: false });

    if (inServiceError) throw new Error(inServiceError.message);

    // 2. Recent events
    const { data: history, error: historyError } = await supabase
      .from('tool_service_history')
      .select('*')
      .order('created_at', { ascending: false })
      .limit(50);
    ...
    // 3. Fetch related tools for history items manually
    const toolIds = [...new Set((history || []).map(h => h.tool_id).filter(Boolean))];
    const toolMap = {};
    ...
    const recentEvents = (history || []).map(h => ({
      ...h,
      tool_name: toolMap[h.tool_id]?.name,
      tool_sku: toolMap[h.tool_id]?.sku,
      name: toolMap[h.tool_id]?.name,
      sku: toolMap[h.tool_id]?.sku
    }));
  ...
  // SERVICE HISTORY - CLEAR ALL (DELETE)
  if (route === '/api/service-history' && method === 'DELETE') {
    ...
    if (error) throw new Error(error.message);
    return { message: `Deleted ${count} records`, deleted: count };
  }

  // CATEGORIES STATS (GET)
  if (route === '/api/categories/stats' && method === 'GET') {
  ...
  if (route === '/api/config/email' && method === 'GET') {
    const { data, error } = await supabase
      .from('app_config')
      .select('smtp_host, smtp_port, smtp_user, smtp_pass, smtp_secure, smtp_from')
      .single();
    
    if (error) {
       // If table doesn't exist or empty, return default structure to avoid UI errors
       return {
         smtp_host: '',
         smtp_port: '',
         smtp_user: '',
