...
import { waitForOnline, reportConnectivity } from './net';
import { showSnackbar } from './snackbar';
import { KEYS, getStorageItem, setStorageItem, removeStorageItem } from './storage';
import { supabase } from '../utils/supabase';
import { handleSupabaseRequest } from './supabaseMapping';
...
function resolveBaseUrl() {
  const source = process.env.EXPO_PUBLIC_API_SOURCE || 'localapi';
  
  if (source === 'supabase') {
    return normalizeBaseUrl(process.env.EXPO_PUBLIC_SUPABASE_URL);
  }

  // localapi logic
  // 1) Explicit local URL from env
  if (process.env.EXPO_PUBLIC_API_LOCAL_URL) {
    return normalizeBaseUrl(process.env.EXPO_PUBLIC_API_LOCAL_URL);
  }
  ...
  if (Platform.OS === 'android') {
    return normalizeBaseUrl('http://192.168.10.97:3000');
  }
...
class ApiClient {
  constructor(baseURL = API_BASE_URL) {
    ...
    this.source = process.env.EXPO_PUBLIC_API_SOURCE || 'localapi';
    this.token = null;
    this._listeners = [];
  ...
  async init() {
    const storedToken = await getStorageItem(KEYS.TOKEN);
  ...
  async setToken(token) {
    this.token = token;
    if (token) {
      await setStorageItem(KEYS.TOKEN, token);
    } else {
  ...
  async request(endpoint, config = {}) {
    if (this.source === 'supabase') {
      ...
      // Pass absolute URL logic if needed, but handleSupabaseRequest handles relative paths too
      return handleSupabaseRequest(endpoint, method, body, headers, supabase);
    }
    ...
    try {
      ...
      if (isNetworkErr) {
        ...
        // Jeśli po wszystkich próbach nadal brak połączenia – wyloguj użytkownika
        // Użytkownik zostanie przeniesiony do ekranu logowania (App.js nasłuchuje zmian tokena)
        await this.setToken(null);
        showSnackbar('Brak połączenia z API. Nastąpiło wylogowanie.', { type: 'error' });
      }
      throw e;
    }
  }
